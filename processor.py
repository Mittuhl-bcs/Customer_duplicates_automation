...
# Create alphabetical sort key that keeps duplicate clusters together
def create_cluster_alphabetical_sort_key(row):
    if row['duplicate_check'] == 'yes':
        # For duplicates: get the alphabetically first name in the cluster for sorting
        cluster_ids = row['primary_group_duplicate_ids']
        cluster_names = df[df['customer_id'].isin(cluster_ids)]['customer_name'].str.strip().str.lower()
        min_name = cluster_names.min() if not cluster_names.empty else 'zzz'
        return (min_name, row['primary_group_id'], row['secondary_group_id'], -row['priority'])
    else:
        # For non-duplicates: sort by their own name
        customer_name = row['customer_name'].strip().lower() if pd.notnull(row['customer_name']) else 'zzz'
        return (customer_name, 999999, 999999, 0)

# Apply the custom sorting
df['sort_key'] = df.apply(create_cluster_alphabetical_sort_key, axis=1)
